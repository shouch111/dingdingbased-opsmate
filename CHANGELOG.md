...
- 全项目 15 个模块、约 112 处 `print()` 全部替换为 `logging` 调用：
  - ✅/普通信息 -> `logger.info`，⚠️ -> `logger.warning`，❌ -> `logger.error`
  - except 块统一用 `logger.exception()`（自动带完整堆栈）
  - 调试细节 -> `logger.debug`（生产 INFO 级别不输出，调试时切 DEBUG）
  - 去掉 emoji 和 `[module]` 前缀，f-string 改为 `%` 风格惰性格式化
