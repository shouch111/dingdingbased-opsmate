...
# 工具执行时写入分配结果，供 ai_process 读取后透传给 postprocess
# 用 contextvars 保证线程池并发隔离（每个 run_in_threadpool 调用链独立）
_assigned_engineer_ctx = contextvars.ContextVar("assigned_engineer", default="")


def get_assigned_engineer() -> str:
    """读取当前请求链路中工具写入的分配结果（供 ai_agent 调用）"""
    return _assigned_engineer_ctx.get()


def reset_assigned_engineer():
    """重置分配标记（ai_process 每次调用前执行）"""
    _assigned_engineer_ctx.set("")
...
def assign_engineer(candidates: str, task_title: str, task_description: str) -> str:
    ...
    try:
        ...
        # 写入请求级上下文，供 ai_process 透传给 postprocess 做结构化状态判定
        _assigned_engineer_ctx.set(chosen)
