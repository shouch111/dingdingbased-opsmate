...
def classify_node(state: AgentState) -> dict:
    ...
    # ===== 第零层：闲聊预筛（非 IT 任务，直接判定 easy）=====
    user_text_lower = user_text.lower()
    for pattern in CASUAL_PATTERNS:
        if pattern.lower() in user_text_lower:
            print(f"[classify] 闲聊命中「{pattern}」→ 直接判定为 easy")
            return {"difficulty": "easy"}
    # =====================================================
...
def assign_node(state: AgentState) -> dict:
    ...
    # 校验 LLM 返回的名字是否在工程师名单中
    valid_names = {e["name"] for e in engineers}
    if engineer_name not in valid_names:
        print(f"[assign] LLM 返回的「{engineer_name}」不在名单中，回退到第一条")
        fallback = engineers[0] if engineers else None
        if fallback:
            engineer_name = fallback["name"]
            reason = f"LLM 匹配异常，已自动分配至 {engineer_name}"
        else:
            engineer_name = "无人匹配"
            reason = "无可用工程师"
